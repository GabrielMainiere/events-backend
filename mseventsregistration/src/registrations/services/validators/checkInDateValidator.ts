...
export class CheckInDateValidator implements ICheckInValidator {
    async validate(_registration: Registration, event: tb_registered_event): Promise<void> {
        ...
        if (today > eventEndDate) {
            throw new Error('Check-in is no longer allowed. The event has already ended.');
        }
    }
}
